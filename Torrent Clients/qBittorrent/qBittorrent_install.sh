...
install_qBittorrent_(){
	...
	wget https://raw.githubusercontent.com/race-in/dedi-components/main/Torrent%20Clients/qBittorrent/$arch/$qb_ver%20-%20$lib_ver/qbittorrent-nox -O $HOME/qbittorrent-nox && chmod +x $HOME/qbittorrent-nox
...
EOF
        ...
        wget  https://raw.githubusercontent.com/race-in/dedi-components/main/Torrent%20Clients/qBittorrent/$arch/qb_password_gen -O $HOME/qb_password_gen && chmod +x $HOME/qb_password_gen
        #Check if the download is successful
		if [ $? -ne 0 ]; then
